...
async def llm_predict_images(cr, images):
    images_base64 = [image_to_base64(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)) for image in images]
    llm_results, tokens = await llm_extract_image(images_base64=images_base64, client_registry=cr)
    return llm_results, tokens


async def llm_predict(uuid, files_name, cr, images=None):

    results, llm_results, tokens = [], None, [0, 0]
    try:
        llm_results, tokens = await llm_predict_images(cr, images)
